...
def get_recent_data(ticker, apikey):
    ...
    req = requests.get(apiurl, params={
        "period_id": "4HRS",
        "limit": 75
    }, headers={
...
def execute_trade(predicted_price, actual_price, balance, btc_balance, constant_data, changing_data, trader, liquidate = False):
...
class Predictor:
    def __init__(self, ticker, lookback, apikey, model, trader: tradeutil.Trader, constant_data):
        self.ticker = ticker
    ...
    def trade(self):
        ...
        df_scaled, scaler = process_data.add_technical_indicators(df_merged, self.lookback, 0)
        
        features = df_scaled[['3_day_avg_price', 'tsi', 'rsi', 'sharpe_ratio', 'Bollinger_Upper', 'Bollinger_Lower']]
        target = df_scaled[["PRICE"]]
        X_data, y_data = process_data.create_dataset(pd.concat([target, features], axis=1), self.lookback)
        ...
        predicted_price_tomorrow = scaler.inverse_transform(np.concatenate((prediction_scaled[len(prediction_scaled) - 1].reshape(-1, 1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]
        ...
        if predicted_change < -0.1:
            balance, btc_balance, changing_data_returned = execute_trade(predicted_price_tomorrow, actual_price_today, balance, btc_balance, self.constant_data, self.changing_data, self.trader, True)
            self.changing_data = changing_data_returned

            print(f'Liquidation executed. {self.ticker} Balance: {round(btc_balance, 5)}, USD Balance: {round(balance, 2)}')
        elif abs(predicted_change) > 0.05:
            balance, btc_balance, changing_data_returned = execute_trade(predicted_price_tomorrow, actual_price_today, balance, btc_balance, self.constant_data, self.changing_data, self.trader)
            self.changing_data = changing_data_returned

            print(f'Trade executed. {self.ticker} Balance: {round(btc_balance, 5)}, USD Balance: {round(balance, 2)}')