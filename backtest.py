...
def execute_trade(predicted_price, actual_price, actual_price_tomorrow, balance, btc_balance, implied_values, liquidate = False):
    ...
    # Sell BTC
        ...
        if gain < 0.1:
          # can't trade less than 10 cents
          return balance, btc_balance, model_was_right
        print(f'{"Selling" if liquidate == False else "Liquidating"} {btc_sold} {ticker} (${round(gain, 2)})')
...
def backtest(model, X_test, y_test, scaler, ticker, window, lookback, num_features, num_features_backtest):
    ...
    for i in range(len(predicted_prices_tomorrow)):
        ...
        actual_price_today = scaler.inverse_transform(np.concatenate((y_test[i].reshape(-1, 1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]

        # see if we can seek 1 forward
        if (i + 1) < (len(y_test) - 1):
            actual_price_tomorrow = scaler.inverse_transform(np.concatenate((y_test[i + 1].reshape(-1, 1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]
        else:
            actual_price_tomorrow = None

        # Check if the predicted change is more than 2%
        predicted_change = (predicted_price_tomorrow - actual_price_today) / actual_price_today
        print(f'Actual price today: {actual_price_today}, Predicted price tomorrow: {predicted_price_tomorrow}')
        print(f'Model predicts market change will be {round(predicted_change * 100, 2)}%')
        if predicted_change < -0.1:
            balance, btc_balance, model_was_right = execute_trade(predicted_price_tomorrow, actual_price_today, actual_price_tomorrow, balance, btc_balance, implied_values, True)
        ...
        # else: the no trade happened
            ...
            print(f'Liquidation executed. {ticker} Balance: {round(btc_balance, 5)}, USD Balance: {round(balance, 2)}')
        elif abs(predicted_change) > 0.05:
            balance, btc_balance, model_was_right = execute_trade(predicted_price_tomorrow, actual_price_today, actual_price_tomorrow, balance, btc_balance, implied_values)
            ...
            print(f'Trade executed. {ticker} Balance: {round(btc_balance, 5)}, USD Balance: {round(balance, 2)}')

        # Implementing stoploss
        if (stoploss_enabled and (btc_balance > 0) and ((btc_usd_balance_if_ending / initial_balance) <= (1 - stoploss))):
            #print(btc_usd_balance_if_ending / initial_balance, btc_balance, 1 - stoploss)
            balance += btc_balance * actual_price_today
            btc_balance = 0
            print(f'Stoploss activated. {ticker} Balance: {round(btc_balance, 6)}, USD Balance: {round(balance, 2)}')
            stoploss = True
            btc_end_override = scaler.inverse_transform(np.concatenate((y_test[i].reshape(-1,1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]
            break
    ...
    print(f'Out of {trades} trades, {trades_buy} were buys and {trades_sell} were sells')

    print()
    print('--- Buy & Hodl ---')

    btc_start = scaler.inverse_transform(np.concatenate((y_test[0].reshape(-1,1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]

    btc_end = 0
    if not btc_end_override:
        btc_end = scaler.inverse_transform(np.concatenate((y_test[len(y_test) - 1].reshape(-1, 1), np.zeros((1, num_features_backtest))), axis=1))[0, 0]
    else:
    ...
    is_outperform = "outperformed" if change_percent > friendly_gainloss_percentage else "underperformed"
    outperform_val = change_percent - friendly_gainloss_percentage

    print(f'The market {is_loss} {friendly_gainloss_percentage}% during the test period.')
    print(f'An investor starting with ${round(initial_balance, 2)} would have ${round(initial_balance * gainloss, 2)} using B&H strategy.')

    print(f'The model {is_outperform} by {round(outperform_val, 2)}% in comparison to the market.')
    ...
    print()
    print(f'The model was right {model_right_amount} times and wrong {model_wrong_amount} times ({round((model_right_amount / (model_right_amount + model_wrong_amount)) * 100, 2)}%)')